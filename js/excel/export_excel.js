...
// let myExcelXML = (function(sheet_name) {
    ...
    let fs, styleID = 1, columnWidth = 50, uri, link;

    class myExcelXML {
        constructor(o, headers) {
        ...
        downLoad(sheet_name) {
            const Worksheet = myXMLWorkSheet(sheet_name, fs, this.headers);
    ...
    const myXMLWorkSheet = function(name, o, headers) {
        const Table = myXMLTable(o, headers);
    ...
    const myXMLTable = function(o, headers) {
        ...
        if (tableData.length > 0) {
            ...
            for (let i = 0; i < columnHeader.length; i++) {
                list.push(columnWidth);
            }
            list[0] = 200;
            list[1] = 200;
            list[columnHeader.length-3] = 100;
            list[columnHeader.length-2] = 100;
            list[columnHeader.length-1] = 100;
            for (let i = 0; i < columnHeader.length; i++) {
                TableStart += myXMLColumn(list[i]);

            }

            // Preparing for rows not headers
